...
$sql = "SELECT * FROM appointmentsdrawinck WHERE sent_to_sendpulse = FALSE";
    ...
    while ($row = $result->fetch_assoc()) {
        $newEmail = [
            ...
            'variables' => [
                ...
                'Teléfono' => $row['phone'],
                'fecha de cita' => $row['appointment_date'],
                'hora de cita' => $row['appointment_time'],
                'servicio' => $row['service'],
                'lugar' => $row['location'],
                'mensaje' => $row['comments'],
        ...
        try {
            $response = $apiClient->post('addressbooks/741322/emails', [
                'emails' => [
                    $newEmail
                ]
            ]);
